...
export default function AccountRecovery() {
  ...
  const HandleRegisterSubmit = async (event) => {
    ...
    if (password == cpassword) {
      const res = await axios.post(
        config.api_ChangePassword,
        { phone: phoneState, otp: otpval, password: password }
      );
      if (res.data.status) {
  ...
  const HandleOtpSubmit = async (event) => {
    ...
    if (otp) {
      const res = await axios.post(
        config.api_ForgotValidateOTP,
        { phone: phoneState, otp: otp }
      );
      if (res.data.status) {
        setotp(otp);
        setisOtpValid(true);
  ...
  const HandleValidatePhone = async (event) => {
    ...
    if (phone) {
      ...
      if (phone.match(phonenoRegEx)) {
        ...
        setphone(phone);
        const res = await axios.post(
          config.api_ValidatePhoneForgot,
          { phone }
        );
        if (res.data.status) {
          //success
          ...
          set_Error({
            status: true,
            detail: 'Phone is not registered with us.',
          });
        }
      } else {
        set_Error({
      ...
      {isRegSuccess ? (
        ...
        >
          ...
          <p className="mt-2 text-center text-sm text-gray-600">
            <p className="mt-2 text-center text-sm text-gray-600">
              Password Reset Comdplete. Please go to Login. <br />
              <Link
                to="/login"
